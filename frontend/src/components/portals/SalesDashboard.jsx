...
export const SalesDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState('retail_orders');
  ...
  useEffect(() => {
    ...
    if (parts.length > 1 && parts[0] === 'sales') {
      setActiveTab(parts.slice(1).join('_'));
    }
  }, []);
  ...
  useEffect(() => {
    const loadQuotes = async () => {
      ...
      const combined = backendQuotes.map(bq => {
        const matchingLocal = localQuotes.find((lq) => lq.id === bq.id);
        if (matchingLocal) {
          return {
            ...bq,
  ...
  const handleQuoteAction = async (id, action, newPrice, remarks) => {
    try {
    ...
    const updated = existing.map((q) => q.id === id ? {
      ...q,
      status: newStatus,
  ...
  const handleUpdatePrice = (id, newPrice, remarks) => {
              ...
              <div className="lg:col-span-5 p-6 rounded-3xl bg-white shadow-sm backdrop-blur-2xl border border-blue-500/40 shadow-2xl space-y-5">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {getIndividualItems(selectedQuote).map((item, idx) => (
                ...
                <form 
                  onSubmit={(e) => {
                    ...
                    const remarks = (form.elements.namedItem('remarks')).value;
