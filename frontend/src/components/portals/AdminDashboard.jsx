...
export const AdminDashboard = () => {
  ...
  const [newProductUnitPrice, setNewProductUnitPrice] = useState(100);
  const [newProductRetailPrice, setNewProductRetailPrice] = useState(279);
  const [newProductGrade, setNewProductGrade] = useState('100% Steam Distilled • Pharmaceutical Grade');
  const [newProductAvailability, setNewProductAvailability] = useState('In Stock');
        ...
        if (res.ok) {
          ...
          if (Array.isArray(data)) {
            backendQuotes = data.map((item) => ({
              ...
              quantity: item.items && item.items.length > 0 ? item.items.map((i) => `${i.quantityKg || i.quantity || 50} KG`).join(', ') : '50 KG',
  ...
  const [editForm, setEditForm] = useState({ price: '', description: '', images: [] });
  ...
  const handleAssignQuote = (quoteId, agentName) => {
  ...
  const handleGenerateShipment = async (orderPk) => {
    if (!token) return;
    try {
                ...
                <form onSubmit={handleCreateProduct} className="space-y-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setNewProductAvailability(e.target.value)}
