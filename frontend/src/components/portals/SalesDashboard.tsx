...
import { 
  ...
  Truck, MapPin, PackageCheck, User, CheckCircle, ExternalLink, LogOut, Mail
} from 'lucide-react';
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(data)) {
          setAssignedCustomers(data.map((c: any) => ({
            id: c.id,
            name: `${c.first_name || ''} ${c.last_name || ''}`.trim() || c.company_name || c.email?.split('@')[0] || 'Direct Client',
            contactPerson: `${c.first_name || ''} ${c.last_name || ''}`.trim() || 'Key Account',
            email: c.email || 'No email provided',
            phone: c.mobile_number || '+91 98765 43210',
            stage: c.customer_stage || (c.orders_count > 0 ? 'Customer' : 'Lead'),
            activeOrders: `${c.orders_count || 0} Orders`,
            totalSpent: c.total_spent ? `₹${Number(c.total_spent).toLocaleString()}` : '₹0'
          })));
        }
      }
    } catch (e) {
      console.error('Failed to load assigned customers:', e);
    } finally {
        ...
        {activeTab === 'customers' && (
          <div className="p-8 rounded-3xl bg-white shadow-sm border border-neutral-200 shadow-xl space-y-6">
            <div>
              ...
              <p className="text-sm text-neutral-500 mt-1">Customers and client accounts assigned directly to your sales profile by the administrator.</p>
            </div>
            ...
            {loadingCustomers ? (
              ...
              <div className="text-center py-16 bg-neutral-50 rounded-3xl border border-neutral-200 space-y-3">
                <Users className="w-12 h-12 text-neutral-400 mx-auto" />
                <h4 className="text-lg font-bold text-neutral-800">No Customers Currently Assigned</h4>
                <p className="text-xs text-neutral-500 max-w-md mx-auto">
                  Your administrator can assign verified buyers and inbound leads to your sales profile from the Admin Portal.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {assignedCustomers.map((c) => (
                  <div key={c.id} className="p-6 rounded-3xl bg-white shadow-sm border border-neutral-200 space-y-4 hover:border-neutral-300 transition-all">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`px-2.5 py-1 rounded-full text-xs font-extrabold uppercase ${
                        c.stage === 'Lead' ? 'bg-amber-50 text-amber-700 border border-amber-200' : 'bg-emerald-50 text-emerald-700 border border-emerald-200'
                      }`}>
                        {c.stage}
                      </span>
                    </div>

                    <div className="text-xs text-neutral-600 space-y-1.5 bg-neutral-50 p-4 rounded-2xl border border-neutral-100">
                      <div><span className="text-neutral-500 font-semibold">Contact Person:</span> {c.contactPerson}</div>
                      <div><span className="text-neutral-500 font-semibold">Email:</span> {c.email}</div>
                      <div><span className="text-neutral-500 font-semibold">Phone:</span> {c.phone}</div>
                      <div><span className="text-neutral-500 font-semibold">Order History:</span> {c.activeOrders} ({c.totalSpent})</div>
                    </div>

                    <div className="pt-2 flex items-center justify-between gap-3">
                      <a 
                        href={`tel:${c.phone}`}
                        className="flex-1 py-2.5 rounded-xl bg-neutral-900 hover:bg-neutral-800 text-white font-bold text-xs flex items-center justify-center gap-2 transition-all cursor-pointer"
                      >
                      ...
                      {c.email && c.email.includes('@') && (
                        <a 
                          href={`mailto:${c.email}`}
                          className="px-4 py-2.5 rounded-xl bg-neutral-100 hover:bg-neutral-200 text-neutral-800 font-bold text-xs flex items-center justify-center gap-2 transition-all cursor-pointer"
                        >
